...
export default function OptimizePage() {
  ...
  // 添加加载提示词的函数
  const loadPromptFromDatabase = async (promptId: string) => {
    try {
      ...
      // 使用数据库中的数据更新状态
      setStreamContent(data.optimized_prompt)
      setEditedContent(data.optimized_prompt)
      setPromptHistory([{
      ...
      setModel(data.model || "deepseek-v3")
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (promptId) {
        // 如果有ID，从数据库加载数据
        await loadPromptFromDatabase(promptId)
      } else {
        // 检查是否有本地存储的历史记录
